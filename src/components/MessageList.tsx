                ...
                <div className="text-xs text-muted-foreground">
                  {(() => {
                    ...
                    if (showStats && m.metrics) {
                      ...
                      const pCost = promptRate != null && pt != null ? (promptRate / 1_000_000) * pt : 0;
...
function RegenerateMenu({ onChoose }: { onChoose: (modelId?: string) => void }) {
  ...
  return (
    <div className="relative">
      <button className="icon-button" aria-label="Regenerate" title="Regenerate" onClick={() => setOpen((v) => !v)}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 z-40 card p-2 w-56">
          ...
          {options.map((o) => (
            <div key={o.id} className="p-2 rounded hover:bg-muted cursor-pointer text-sm" onClick={() => { onChoose(o.id); setOpen(false); }}>
              {o.name || o.id}
            </div>
          ))}
          <div className="border-t border-border my-1" />
          <div className="flex gap-1">
            <input className="input flex-1" placeholder="provider/model-id" value={custom} onChange={(e) => setCustom(e.target.value)} />
            <button className="btn btn-outline" onClick={() => { const id = custom.trim(); onChoose(id || undefined); setOpen(false); }}>Go</button>
          </div>
        </div>
      )}
    </div>
