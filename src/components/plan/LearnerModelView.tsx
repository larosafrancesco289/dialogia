...
export function LearnerStats({ learnerModel, plan }: { learnerModel?: LearnerModel; plan: LearningPlan }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      ...
      <div className="p-3 rounded-xl border border-border bg-surface/50 space-y-1 shadow-sm">
        ...
        <div className="text-xl font-bold text-foreground">
          {masteredTopics} <span className="text-sm font-normal text-muted-foreground">/ {plan.nodes.length}</span>
        </div>
      </div>
      ...
      <div className="p-3 rounded-xl border border-border bg-surface/50 space-y-1 shadow-sm">
        ...
        <div className="text-xl font-bold text-foreground">
          {totalInteractions} <span className="text-sm font-normal text-muted-foreground">interactions</span>
        </div>
      </div>
    </div>
  );
...
export function LearnerInsights({ learnerModel, plan }: { learnerModel?: LearnerModel; plan: LearningPlan }) {
  if (!learnerModel) {
    return (
  ...
  const interestingTopics = plan.nodes.filter(node => {
          ...
          return (
            ...
            >
              ...
              {mastery?.misconceptions && mastery.misconceptions.length > 0 && (
               ...
               {(!mastery?.misconceptions || mastery.misconceptions.length === 0) && mastery?.evidence && (
