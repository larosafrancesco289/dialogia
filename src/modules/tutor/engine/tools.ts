...
const ARGS = {
  ...
  record_evidence: z.object({
    ...
    helped: z
      ...
      .optional()
      .describe(
        "true only when your message just before did this answer's step for them on this very problem (named the operation to use, gave its first step, pointed at the part to fix, put the answer in the question or its choices), or they repeated your correction of the same problem back. false for their answer to a fresh problem you posed, even right after correcting or explaining the rule: applying it to a new case is their own work. The estimate then moves less (partial: not at all).",
      ),
    source: z
...
const DESCRIPTIONS: Record<TutorToolName, string> = {
  ...
  record_evidence: `Record what the conversation showed about the learner's grasp of a topic, judged from their own words: struggled (an answer with an error in it, even if part was right, or stuck), partial (right as far as it went, but incomplete), applied (used it correctly), explained (explained it back), insight (went beyond what was taught). helped: only when your last message did the step for them; an answer to a fresh problem you posed is their own. One call per topic per reply, summing up the exchange; a reply that notes a misconception on a topic gains nothing on it. Source "learner_said" when they tell you about their own understanding. Not for quiz or diagnostic answers. Does not end your turn.`,
