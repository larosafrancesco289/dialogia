...
import type { TutorPhase } from '@/modules/tutor/agent/types';
import { advanceTopicTool } from '@/modules/tutor/tools/definitions/advanceTopic';
import { askStudentQuestionTool } from '@/modules/tutor/tools/definitions/askStudentQuestion';
import { createDiagnosticTool } from '@/modules/tutor/tools/definitions/createDiagnostic';
import { learningPlanTool } from '@/modules/tutor/tools/definitions/learningPlan';
import { quizTool } from '@/modules/tutor/tools/definitions/quiz';
import { recordLearningTool } from '@/modules/tutor/tools/definitions/recordLearning';
...
const createTutorHandler = (name: TutorToolName, kind: 'content' | 'meta'): PlanningToolHandler => {
  return async ({ toolCall, parsedArgs, roundMeta, context, aggregatedResults }) => {
    ...
    const { applyTutorToolCall, recordTutorToolUsage } = await import(
      '@/modules/tutor/tools/apply'
    );
